...
# automation of certificate renewal for let's encrypt and nginx
# - checks all certificates under /etc/letsencrypt/live and renews
#   those about about to expire in less than 4 weeks
# - creates ssl certificate files in /etc/letsencrypt/live/domain.tld/
# - reloads nginx to apply new certificates
# usage:
# sudo ./cert-renewal-nginx.sh
...
NGINX_RELOAD_CMD="supervisorctl signal HUP nginx"
NGINX_SOFTSTOP_CMD="supervisorctl signal USR1 nginx"
...
# create nginx.pem file(s)
for domain in ${renewed_certs[@]}; do
  cat ${le_cert_root}/${domain}/privkey.pem ${le_cert_root}/${domain}/fullchain.pem | tee /etc/nginx/certs/nginx-${domain}.pem >/dev/null
  if [ $? -ne 0 ]; then
    logger_error "failed to create nginx.pem file!"
    exit 1
  fi
done

# soft-stop (and implicit restart) of nginx
# (reload command does not reload certs)
if [ "${#renewed_certs[@]}" -gt 0 ]; then
  $NGINX_SOFTSTOP_CMD
  if [ $? -ne 0 ]; then
    logger_error "failed to stop nginx!"
    exit 1
  fi
fi
